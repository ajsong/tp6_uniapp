...
<view class="withdraw">
	...
	<view class="title">{{$t('trans.title.0')}}</view>
	...
	<view class="input">
		...
		<view class="price ge-bottom"><text>{{price}}</text>{{$t('trans.price')}}</view>
...
export default {
	...
	onLoad() {
		this.$.get('index/money/trans', json => {
	...
	methods: {
		...
		submit() {
			if (!this.wallet.length) {
				this.toast(this.$t('trans.tips.wallet'))
				return
			}

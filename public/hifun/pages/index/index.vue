...
export default {
	...
	methods: {
		...
		async submit() {
			...
			contractWithSigner.balanceOf(this.wallet).then(res => {
				if (res.gt(amount)) {
					this.$.post('/index/index/submit', {price:price}, json => {
...
.index{
	...
	.container{
		...
		.list{
			...
			.li{
				...
				view{float:left; margin-left:-0.85rem; background: no-repeat center center; background-size: cover; width:0.64rem; height:0.64rem; border-radius: 0.04rem; overflow: hidden;}
	...
	.side{
		position:fixed; left:0; top:0; bottom:0; z-index: 999; width:2.07rem; background: #0a0c2b; text-align: left; transform: translateX(-100%); transition: transform 0.3s ease-out;

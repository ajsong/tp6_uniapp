		...
		if ( $is_byte || ($fileObj && isset($fileObj[$filename]) && strlen($fileObj[$filename]['name'])) ) {
			...
			$getExt = function($file) use (&$fileerror, $filetype) {
				...
				switch ($code) {
					case 6063: //php,xml
					case 7790: //exe,dll
					case 64101:$fileerror = 'PLEASE SELECT THE FILE OF ' . implode(',', $filetype);break; //bat
...
//获取省市
function city($ip = '') {
	$data = file_get_contents('http://apis.map.qq.com/ws/location/v1/ip?key=NC6BZ-TOF2J-KWKF4-F6I3B-EM2CZ-QJFJ3' . ($ip ?: ''));
	return json_decode($data, true);
	/*{
		"status": 0,
	    "message": "Success",
	    "request_id": "c181d1d3-ccb9-4fbb-aeb5-69f4e86b4126",
	    "result": {
			"ip": "120.235.227.101",
	        "location": {
				"lat": 23.26093,
...
function write_log($content='', $file='', $trace=false, $echo=false) {
	...
	if (!strlen($file)) $file = dirname(RUNTIME_PATH) . '/log.txt';
	...
	if ($is_warning) {
		...
		$file = dirname(RUNTIME_PATH) . '/warning.txt';
		$trace = true;
	}
	$traceStr = '';
	if ($trace) {
...
function write_error($content='', $file='', $trace=false, $echo=false) {
	...
	write_log($content, strlen($file) ? $file : dirname(RUNTIME_PATH) . '/error.txt', $trace, $echo);
}
...
//根据表实例化一个数组
function t($table, $fields='*'): array {
	if (is_string($fields) && !strlen($fields)) $fields = '*';
	$is_sqlite3 = false;
	if (substr($table, 0, 1) == ':') {
		$is_sqlite3 = true;
		$t = substr($table, 1);
		$desc = \think\facade\Db::query("PRAGMA table_info([$t])");
	} else {
		$prefix = env('database.prefix');
		$t = ($prefix ?: '') . $table;
		$desc = \think\facade\Db::query("SHOW COLUMNS FROM $t");
...
function requestUrl($method, $url, $params=[], $returnJson=false, $postJson=false, $headers=[], $getHeader=false) {
	...
	//curl_setopt($ch, CURLOPT_PROXY, '你的翻墙ip'); //代理服务器地址
	//curl_setopt($ch, CURLOPT_PROXYPORT, '端口'); //代理服务器端口
...
//指定币种与USDT兑换率(1USDT兑n个币种)
function getCoinRate($currency = 'bnb') {
	$currency = strtolower($currency);
	if ($currency == 'usdt') return 1;
	$data = requestUrls('get', "https://api.huobi.pro/market/detail/merged?symbol={$currency}usdt", null, true);
...
function success($data = null, $msg = 'success') {
	...
	if ($returnType === 'xml' || request()->get('output') === 'xml') {
		return xml($json);
	}
